...
describe('Registering a new user', () => {
  test('with valid details', async () => {
    ...
    const userAuth = await prismaService.auth.findUnique({
...
describe('Tokens refresh', () => {
  ...
  test('with an expired refresh token', async () => {
    ...
    await cacheManager.set(
      payload.sub,
      tokens,
      1000 * configService.get<number>('jwt.refreshTokenExpiresIn')!,
    );
    await expect(authService.refreshTokens(tokens)).rejects.toThrow(
  ...
  test('with cross user tokens', async () => {
    ...
    const loginResult = await authService.login({
      email: user2.email,
      password: newUserSignupDto.password,
    });
...
describe('Resetting user password', () => {
  ...
  test('for a non-existing user', async () => {
    ...
    await expect(
      authService.resetPassword(passwordResetDto, faker.string.uuid()),
  ...
  test('with an incorrect old password', async () => {
    ...
    await expect(
      authService.resetPassword(passwordResetDto, user.id),
  ...
  test('when old password equals new password', async () => {
    ...
    await expect(
      ...
      new BadRequestException('Old and nwew password cannot be the same!'),
    );
  });
});
...
describe('Fetching user profile', () => {
  ...
  test('for a non-existing user', async () => {
    await expect(
      authService.profile(faker.string.alphanumeric(20)),
    ).rejects.toThrow(new NotFoundException('User not found!'));
  });
});
