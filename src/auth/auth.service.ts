...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    await this.cacheManager.set(
      auth.userId,
      tokens,
      1000 * this.configService.get<number>('jwt.refreshTokenExpiresIn')!,
    );
  ...
  async refreshTokens(refreshTokensDto: RefreshTokensDto) {
    try {
      ...
      const cachedTokens = await this.cacheManager.get<AuthTokens>(
        refreshPayload.sub,
      );
      const isTokenMatch =
        cachedTokens?.refreshToken === refreshTokensDto.refreshToken &&
        cachedTokens?.accessToken === refreshTokensDto.accessToken;
  ...
  async resetPassword(passwordResetDto: PasswordResetDto, userId: string) {
    ...
    if (oldPassword === newPassword)
      throw new BadRequestException(
        'Old and nwew password cannot be the same!',
      );
